...
def apply_calibration(df_in, str_expt):
    ...
    x = np.array(df_in['temperature_CS']).reshape(-1, 1)
    y = np.array(df_in['temperature_Op']).reshape(-1, 1)
    ...
    plt.plot(x, corrected_y, color='green', lw=2, label='Calibrated Curve')
